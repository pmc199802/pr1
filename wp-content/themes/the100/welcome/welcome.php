<?php
if(!class_exists('The100_Welcome')) :

	class The100_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' => esc_html__('Getting Started', 'the100'),
					'recommended_plugins' => esc_html__('Recommended Plugins', 'the100'),
					'demo_import' => esc_html__('Import Demo', 'the100'),
					'support' => esc_html__('Support', 'the100'),
					'changelog' => esc_html__('Changelog', 'the100'),
					'more_wp' => esc_html__('More WordPress Stuff', 'the100'),
				);
			...
			public function welcome_admin_notice_display(){
				...
				$the100_an_div = '
				<div class="updated the100-an notice notice-success is-dismissible">
				%1$s
				<hr/>
				<div class="the100-column-wrap">
				<div class="the100-column col-img">%2$s</div>
				<div class="the100-column col-demos">%3$s</div>
				<div class="the100-column col-doc">%4$s</div>
				</div> 
				</div>
				<style>%5$s</style>';

				/* translators: 1 - notice title, 2 - notice message */
				$the100_an_top = sprintf(
					'<h2>%1$s</h2><p class="about-desc">%2$s</p></hr>',
					esc_html__( 'Congratulations!', 'the100' ),
					sprintf(
						/* translators: %s - theme name */
						esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'the100' ),
						$this->theme_name
					)
				);

				$the100_an_btn = sprintf(
					/* translators: 1 - url, 2 - button text */
					'<a href="%1$s" class="button button-primary the100-btn" >%2$s</a>',
					esc_url( admin_url( 'themes.php?page=welcome-page#demo_import' ) ),
					esc_html__( 'Try one of our ready to use Starter Sites', 'the100' )
				);
				$the100_an_pbtn = sprintf(
					...
					esc_html__( 'or go to the theme settings', 'the100' )
				);
				$the100_an_img = sprintf(
					'<picture>
				...
				$the100_an_list = sprintf(
					'<div><h3><span class="dashicons dashicons-images-alt2"></span> %1$s</h3><p>%2$s</p></div><div> <p>%3$s</p><p>%4$s</p> </div>',
					__( 'Sites Library', 'the100' ),
					// translators: %s - theme name
					sprintf( esc_html__( '%s now comes with a sites library with various designs to pick from. Visit our collection of demos that are constantly being added.', 'the100' ), $this->theme_name ),
					$the100_an_btn,
					$the100_an_pbtn
				);
				$the100_an_doc = sprintf(
					'<div><h3><span class="dashicons dashicons-format-aside"></span> %1$s</h3><p>%2$s</p><a class="button the100-btn" href="%3$s">%4$s</a></div><div></div>',
					__( 'Documentation', 'the100' ),
					// translators: %s - theme name
					sprintf( esc_html__( 'Need more details? Please check our full documentation for detailed information on how to use %s.', 'the100' ), $this->theme_name ),
					'https://8degreethemes.com/documentation/the100',
					esc_html__( 'Read full documentation', 'the100' )
				);
				$the100_an_style = '
				.the100-an.notice.is-dismissible {padding: 20px 15px 5px;}
				.the100-an hr {margin: 20px -23px 0;border-top: 1px solid #f3f4f5;border-bottom: none;}
				.the100-an h3{margin: 17px 0 0;font-size: 16px;line-height: 1.4;}
				.the100-an p {color: #72777c;}
				.the100-an h2{margin: 0;font-size: 21px;font-weight: 400;line-height: 1.2;}
				.the100-an p.about-desc{color: #72777c;font-size: 16px;margin: 0;padding:0px;}
				.the100-column-wrap {display: -ms-grid;display: grid;-ms-grid-columns: 24% 32% 32%;grid-template-columns: 24% 32% 32%;margin-bottom: 13px;}
				.the100-column {padding: 20px 40px 0 0;}
				.the100-an .the100-btn {padding: 30px;line-height: 0;}';
				...
				<div class="wrap about-wrap access-wrap">					
					<div class="top-block-wrap">					
						<div class="about-text">
							<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'the100' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
						...
						<div class="welcome-section-wrapper is_loading">
							...
							<div class="notice-sidebar is_loading">
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Join in our social networks!','the100') ?></h4>
									<p><?php echo esc_html__('Get connected, share your opinions and more via our social community:', 'the100'); ?></p>
									<p><a href="<?php echo esc_url('https://www.facebook.com/8DegreeThemes');?>"><?php echo esc_html('Join our Facebook Group','the100') ?></a> <?php echo esc_html('- to receive updates, offers and more.','the100') ?></p>
									<p><a href="<?php echo esc_url('https://www.youtube.com/8degreethemes');?>"><?php echo esc_html('Subscribe our YouTube Channel','the100') ?></a> <?php echo esc_html('- for tutorials, videos and more.','the100') ?></p>
									<p><a href="<?php echo esc_url('https://www.twitter.com/8degreethemes');?>"><?php echo esc_html('Follow us on Twitter','the100') ?></a> <?php echo esc_html('- to stay updated.','the100') ?></p>
								</div>
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Leave us a review','the100') ?></h4>
									<p><?php echo esc_html__('Are you are enjoying The100? We would love to hear your feedback.', 'the100'); ?></p>
									<a href="<?php echo esc_url('https://wordpress.org/support/theme/the100/');?>"><?php echo esc_html('Submit a review','the100') ?></a>
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'the100');
				}
