...
function the100_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		...
		array(
			...
			'choices' => array(
				...
				'template6' => __('Agency Demo Layout', 'the100'),
				)
			)
		);	
	...
	$wp_customize->get_section('colors')->title = __( 'Colors & Dark Mode Settings', 'the100' );

	$wp_customize->add_setting( 'the100_dark_mode_setting', array(
	 		'sanitize_callback' => 'the100_sanitize_checkbox'
	));
	$wp_customize->add_control( 'the100_dark_mode_setting', array(
		  	'section'      => 'colors',
		  	'label'        => __( 'Dark Mode Support', 'the100' ),
			'description' => __('This website uses Local Storage to save the setting when Dark Mode support is turned enabled or disabled.<br> Local Storage is necessary for the setting to work and is only used when a user enabled Dark Mode button.<br> No data is saved in the database or transferred.', 'the100'),
		  	'type'         => 'checkbox'
	));
