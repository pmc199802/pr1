...
add_action('the100_abovefooter_section','the100_abovefooter_section_cb', 10);
 ...
 $enable_dark_mode = get_theme_mod('the100_dark_mode_setting',0);
	if($enable_dark_mode){
	add_action( 'wp_footer', 'the_100_dark_switch_display' );
	}
function the_100_dark_switch_display( $attrs = array() ) {
		$attrs = wp_parse_args(
			$attrs,
			array(
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'the100' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'the100' ); ?>';
			}
