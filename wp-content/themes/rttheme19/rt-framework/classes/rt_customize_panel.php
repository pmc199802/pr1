...
class RTFramework_Customize_Panel extends RTTheme
{
	...
	public function load_fonts()
	{
			...
			if( ! empty( $rt_custom_fonts ) ){
				...
				$this->fonts["#2_obt_start"] = esc_html__("Custom Fonts",'rt_theme');
			...
			$this->fonts["#1_obt_start"] = esc_html__("Web Safe Fonts",'rt_theme');
			...
			$google_fonts = array();
				...
				if ( $json ){
					...
					$this->fonts["#3_obt_start"] = esc_html__("Google Fonts",'rt_theme');
	...
	public function customize_admin_js() {
		...
		//localize js params
		$js_params = array(
			"apply_skin" => esc_html__('Do you want to apply this skin? ','rt_theme'), 
			"select_demo" => esc_html__('Please select a demo ','rt_theme'), 
			"wait_previous_install" => esc_html__('An installer has already been started. Please wait!','rt_theme'), 
			"install_revslider" => esc_html__('Install and activate Revolution Slider plugin to proceed.','rt_theme'), 			
			"theme_slug" => RT_THEMESLUG
		);

		wp_localize_script( 'customizer-rt-skins-js', 'rt_theme_params', $js_params );
