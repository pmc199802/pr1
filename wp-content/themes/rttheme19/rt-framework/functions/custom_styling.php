...
$grouped_selectors= array(
	...
	"light-text-color" => array("use"=>"light_text_color","selectors"=>'
		...
		input[type="submit"],
		input[type="button"],
		form button:not(.button_),
		form button:not(.button_):hover,
		...
		?>
			...
			<div class="customizer-notification">
				...
				<div class="customizer-notification-text">
					<strong><?php echo esc_html_x("The current page has some individual design settings different than the global settings of the customizer window. (e.g. header color.)",'Admin Panel', 'rt_theme')."</strong>";?></strong><br />
					<?php echo esc_html_x("As a result, the page may look different than the rest of the website. To prevent confusing while customizing your page and be sure about your changes, just click another link of your website which has no individual customizations.",'Admin Panel', 'rt_theme');?><br />
					<?php echo esc_html_x("Tip: If you don't see this message on a page while the customizer window is open, you can be sure that that page complately follows the global settings (customizer). ",'Admin Panel', 'rt_theme');?><br />
					<?php echo esc_html_x("You can control the individual design options of a page/post from the 'Design Options' box inside the edit screen of the page or post.",'Admin Panel', 'rt_theme');?>
				</div>
			</div>
		<?php
	}
