...
if( ! function_exists("rt_blog_post_loop") ){
	...
	function rt_blog_post_loop( $wp_query = false, $atts = array() ) {
		...
		if ( $wp_query->have_posts() ){
			...
			//open the wrapper
			echo "\n".'<div id="'.sanitize_html_class($id).'" class="blog_list clearfix '.trim($add_holder_class." ".$class).'" data-column-width="'. $column_count .'">'."\n";
...
if( ! function_exists("rt_body_class_name") ){
	...
	function rt_body_class_name($classes) {
		...
		$classes[] = $rt_global_variables["overlapped_header"] == "overlapped" ? "overlapped-header" : "";
 		...
 		// side panel classes
		$classes[] = get_theme_mod( RT_THEMESLUG.'_show_side_panel' ) && get_theme_mod( RT_THEMESLUG.'_hide_side_panel_mobile' ) ? "hide-side-panel-mobile" : "";
		$classes[] = get_theme_mod( RT_THEMESLUG.'_show_side_panel' ) && get_theme_mod( RT_THEMESLUG.'_hide_side_panel_desktop' ) ? "hide-side-panel-desktop" : "";
...
if( ! function_exists("rt_post_meta") ){
	...
	?>
		...
		<div class="post_data">
			...
			<!-- comments -->
			<span class="icon-chat-empty comment_link"><a href="<?php comments_link(); ?>" title="<?php comments_number( esc_html_x('0 Comment','Frontend Text','rt_theme'), esc_html_x('1 Comment','Frontend Text','rt_theme'), esc_html_x('% Comments','Frontend Text','rt_theme') ); ?>" class="comment_link"><?php comments_number( esc_html_x('0 Comment','Frontend Text','rt_theme'), esc_html_x('1 Comment','Frontend Text','rt_theme'), esc_html_x('% Comments','Frontend Text','rt_theme') ); ?></a></span>
			<?php endif;?>
...
if( ! function_exists("rt_content_container") ){
	...
	function rt_content_container($atts) {
		...
		if( isset( $atts["sidebar"] ) ){
			...
			$atts["sidebar"] =  empty( $sidebar_contents ) ? "" : $atts["sidebar"];
		}
...
if( ! function_exists("rt_sub_page_header_function") ){
	...
	function rt_sub_page_header_function() {
		...
		//title
		$title = ! $rt_global_variables["hide_page_title"] ? sprintf('<section class="page-title"><%2$s>%1$s</%2$s></section>', apply_filters( "rt_sub_header_content_title", rt_get_title() ), apply_filters( "rt-page-title-tag", "h1" ) ) : "";
		...
		*/
		...
		$output .= "\n\t".$parallax;
		$output .= "\n\t".apply_filters( "rt_sub_header_content_output", $content_output );
		$output .= "\n".'</div>'."\n";
...
if( ! function_exists("rt_get_title") ){
	...
	function rt_get_title() {
		...
		//search
		if ( is_search() ) {
			$title = sprintf( esc_html_x( 'Search Results for: %s','Frontend Text','rt_theme' ), get_search_query() );
		}
		...
		if ( is_archive() ){
			if ( is_day() ) {
				$title = sprintf( esc_html_x( 'Daily Archives: %s','Frontend Text','rt_theme' ), get_the_date() );
			} elseif ( is_month() ) {
				$title = sprintf( esc_html_x( 'Monthly Archives: %s','Frontend Text','rt_theme' ), get_the_date( esc_html_x( 'F Y','Frontend Text','rt_theme' ) ) );
			} elseif ( is_year() ) {
				$title = sprintf( esc_html_x( 'Yearly Archives: %s','Frontend Text','rt_theme' ), get_the_date( esc_html_x( 'Y','Frontend Text','rt_theme' ) ) );
			} elseif ( is_author() ) {
				$title = sprintf( esc_html_x( 'All posts by: %s','Frontend Text','rt_theme' ), get_the_author()  );
			} elseif ( is_tag() ) {
				$title = sprintf( esc_html_x( 'Tag Archives: %s','Frontend Text','rt_theme' ), single_tag_title( '', false ) );
			}
		}
			...
			if(is_array($rt_attached_documents)){
				...
				foreach($rt_attached_documents as $a_file){
					...
					if(strpos($fileURL, ".doc")){
						$file_text = esc_html_x('Download Word File','Frontend Text','rt_theme');
					}

					elseif(strpos($fileURL, ".xls")){
						$file_text = esc_html_x('Download Excel File','Frontend Text','rt_theme');
					}

					elseif(strpos($fileURL, ".pdf")){
						$file_text = esc_html_x('Download PDF File','Frontend Text','rt_theme');
					}

					elseif(strpos($fileURL, ".ppt")){
						$file_text = esc_html_x('Download PowerPoint File','Frontend Text','rt_theme');
					}

					else{
						$file_text = esc_html_x('Download File','Frontend Text','rt_theme');
					}
...
if( ! function_exists("rt_create_carousel") ){
	...
	function rt_create_carousel( $contents = array(), $atts = array() ){
		...
		//create final output
		$output = sprintf('
				<div id="%1$s" class="rt-carousel rt-image-carousel carousel-holder clearfix %2$s" data-item-width="%4$s" data-nav="%5$s" data-dots="%6$s" data-margin="%8$s" data-autoplay="%9$s" data-timeout="%10$s" data-loop="%11$s" data-tablet-item-width="%12$s" data-mobile-item-width="%13$s">
...
if( ! function_exists("rt_create_photo_gallery") ){
	...
	function rt_create_photo_gallery( $atts ){
		...
		echo ! empty( $gallery_holder_output ) ? $gallery_holder_output : ""; 
...
if( ! function_exists("rt_shortcut_buttons") ){
	...
	?>
		<!-- shortcut buttons -->
		<div id="tools" class="sidebar-widgets" data-item-count="<?php echo esc_attr($button_count);?>">
			...
			<ul>
				...
				?>
					...
					<?php if ( is_user_logged_in() ) { ?>
						<li class="tool-icon" title="<?php echo esc_html_x('Your Account','Frontend Text','rt_theme'); ?>"><span class="icon-user"></span>
							<div class="widget">

								<h5><?php echo esc_html_x('Your Account','Frontend Text','rt_theme'); ?></h5>

								<p>
									<?php
										global $current_user;
										printf( esc_html_x( 'Hello %1$s','Frontend Text','rt_theme' ), '<strong>'. esc_html( $current_user->display_name ) .'</strong>' );
									?>
									<br />
									<?php
										printf( '<a href="%1$s" title="%2$s">%2$s</a> | <a href="%3$s" title="%4$s">%4$s</a>', 
											get_permalink( rt_wpml_translated_page_id( get_option('woocommerce_myaccount_page_id') ) ), 
											esc_html_x('account page','Frontend Text','rt_theme' ), 
											wp_logout_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ), 
											esc_html_x('logout','Frontend Text','rt_theme' ) 
										);
									?>
								</p>
					...
					<?php } else { ?>
						<li class="tool-icon" title="<?php echo esc_html_x('Login','Frontend Text','rt_theme'); ?>"><span class="icon-login"></span>

							<div class="widget">
								<h5><?php echo esc_html_x('Login','Frontend Text','rt_theme'); ?></h5>

									<?php if ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' ): ?>
										<?php echo esc_html_x("Not registered yet?",'Frontend Text','rt_theme')?> <a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php echo esc_html_x('Register','Frontend Text','rt_theme'); ?>"><?php echo esc_html_x('Register','Frontend Text','rt_theme'); ?></a></p>
									<?php endif;?>
										...
										<form method="post" class="login" action="<?php echo get_permalink(get_option( 'woocommerce_myaccount_page_id' )); ?>">
											...
											<p class="form-row form-row-wide">
												<label for="username"><?php echo esc_html_x( 'Username or email address','Frontend Text', 'rt_theme' ); ?> <span class="required">*</span></label>
												<input type="text" class="input-text" name="username" id="username" value="<?php if ( ! empty( $_POST['username'] ) ) echo esc_attr( $_POST['username'] ); ?>" />
											</p>
											<p class="form-row form-row-wide">
												<label for="password"><?php echo esc_html_x( 'Password','Frontend Text', 'rt_theme' ); ?> <span class="required">*</span></label>
											...
											<p class="form-row">
												...
												<input type="submit" class="button" name="login" value="<?php echo esc_html_x( 'Login','Frontend Text', 'rt_theme' ); ?>" />
												<label for="rememberme" class="inline">
													<input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <?php echo esc_html_x( 'Remember me','Frontend Text', 'rt_theme' ); ?>
												</label>
											</p>
											<p class="lost_password">
												<a href="<?php echo esc_url( wc_lostpassword_url() ); ?>"><?php echo esc_html_x( 'Lost your password?','Frontend Text', 'rt_theme' ); ?></a>
											</p>
					...
					<li class="tool-icon cart" title="<?php echo esc_html_x('Cart','Frontend Text','rt_theme'); ?>"><span class="icon-basket"><sub class="number empty"><?php echo esc_html($woocommerce->cart->cart_contents_count);?></sub></span>
						<div class="widget woocommerce widget_shopping_cart">

							<h5><?php echo esc_html_x('Cart','Frontend Text','rt_theme'); ?></h5>
				...
				if( $show_search):
				?>
					<li class="tool-icon search" title="<?php echo esc_html_x('Search','Frontend Text','rt_theme'); ?>"><span class="icon-search"></span>
						<div class="widget">
							<?php
								the_widget('WP_Widget_Search');
							?>
				...
				if( $show_wpml_langs):
				?>
					<li class="tool-icon languages" title="<?php echo esc_html_x('Languages','Frontend Text','rt_theme'); ?>"><span class="icon-globe"><sub><?php echo ( 2 < $button_count ) ? ICL_LANGUAGE_CODE : ICL_LANGUAGE_NAME;?></sub></span>
						<div class="widget">
							<h5><?php echo esc_html_x('Languages','Frontend Text','rt_theme'); ?></h5>
...
if( ! function_exists("rt_top_shortcut_buttons") ){
	...
	?>
		...
		<div id="tools">
			<ul>
				<li class="tool-icon" title="<?php echo esc_html_x('Open','Frontend Text','rt_theme'); ?>"><span class="icon-plus"></span></li>
			</ul>
			<ul>
				...
				if( $show_wpml):
				?>
					<li class="tool-icon languages" title="<?php echo esc_html_x('Languages','Frontend Text','rt_theme'); ?>"><span class="icon-globe"><sub><?php echo ICL_LANGUAGE_CODE ;?></sub></span>
						<div class="widget">
							<h5><?php echo esc_html_x('Languages','Frontend Text','rt_theme'); ?></h5>
							<?php rt_wpml_languages_list();?>
						</div>
...
add_filter( 'oembed_dataparse', 'rtframework_custom_oembed_filter', 0, 4 ) ; 
...
if( ! function_exists("rtframework_main_navigation") ){
	...
	function rtframework_main_navigation(){
		...
		if ( $custom_main_menu != "" ){ // check if the current post has a custom menu
			...
			$menuVars = array(
				'menu'            => esc_html__('Main Navigation','rt_theme'),  
				'menu_id'         => "navigation",
				'class'           => "menu",
				'echo'            => false,
				'container'       => 'nav',  
...
if( ! function_exists("rtframework_second_main_navigation") ){
	...
	function rtframework_second_main_navigation(){
		...
		if ( has_nav_menu( 'rt-theme-second-main-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's location
			...
			$menuVars = array(
				'menu'            => esc_html__('Second Main Navigation','rt_theme'),  
				'menu_id'         => "second-navigation",
				'class'           => "menu",
				'echo'            => false,
				'container'       => 'nav',
...
if( ! function_exists("rtframework_wpml_button") ){
	...
	function rtframework_wpml_button(){
		...
		echo '<li><a href="#" class="rt-wpml-menu-button" title="'.esc_html_x("Switch the language",'Frontend Text',"rt_theme").'"><span>'.ICL_LANGUAGE_CODE.'</span></a></li>'."\n";
...
if( ! function_exists("rtframework_hamburger_search_button") ){
	...
	function rtframework_hamburger_search_button(  $number ){
		...
		echo '<li><a href="#" class="rt-search-button" title="'.esc_html_x("Search",'Frontend Text',"rt_theme").'"><span class="icon-search"></span></a></li>'."\n";
	}
}
...
if( ! function_exists("rtframework_side_panel_button") ){
	...
	function rtframework_side_panel_button(  $number ){
		...
		echo '<li><a href="#" class="rt-menu-button"><span></span><span></span><span></span></a></li>'."\n";
...
if( ! function_exists("rtframework_side_panel_navigation") ){
	...
	function rtframework_side_panel_navigation(){
		...
		if ( $custom_side_panel_menu != "" ){ // check if the current post has a custom menu
			...
			$menuVars = array(
				'menu'            => esc_html__('Side Panel Navigation','rt_theme'),
				'menu_id'         => "rt-side-navigation",
				'class'           => "side-menu",
				'echo'            => false,
				'container'       => '',
...
if( ! function_exists("rtframework_side_panel_languages") ){
	...
	function rtframework_side_panel_languages(){
		...
		echo '<h5>'.esc_html__("SWITCH THE LANGUAGE","rt_theme").'</h5>';
...
if( ! function_exists("rtframework_side_panel_cart") ){
	...
	function rtframework_side_panel_cart(){
		...
		echo '<h5>'. esc_html_x('Cart','Frontend Text','rt_theme') .'</h5>'."\n";
