...
if ( ! function_exists( 'get_woocommerce_page_title' ) ) {
	...
	function get_woocommerce_page_title() {
		...
		if ( is_search() ) {
			$page_title = sprintf( esc_html_x( 'Search Results: &ldquo;%s&rdquo;','Frontend Text', 'rt_theme' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( esc_html_x( '&nbsp;&ndash; Page %s','Frontend Text', 'rt_theme' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'cart_total_items' ) ) {
	...
	function cart_total_items( $fragments ) {
		...
		ob_start(); 
		?>	
			<sub class="number <?php echo ( 0 == $woocommerce->cart->cart_contents_count ) ? "empty" : "";?>"><?php echo esc_html($woocommerce->cart->cart_contents_count);?></sub>
		<?php
...
if ( ! function_exists( 'woocommerce_taxonomy_archive_description' ) ){
	...
	function rt_get_product_search_form() {
		$form = '<form role="search" method="get" id="searchform" class="rt_form" action="' . esc_url( home_url( '/'  ) ) . '">
			<div>
				<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . esc_html_x( 'Search for a product&hellip;','Frontend Text', 'rt_theme' ) . '" />
