...
<div class="row border_grid <?php echo esc_attr($content_width != "1/1" ? 'fixed_heights' : '');?> single-products" itemscope itemtype="http://schema.org/Product">
		...
		<div class="product-summary col <?php echo rt_column_class( $content_width ) ?> col-xs-12">
				 ...
				 */
				?>
				<div class="row <?php echo esc_attr($content_width == "1/1" ? 'fixed_heights' : '');?> ">
					...
					<?php endif;?>
							...
							<?php 
								 ...
								 */
								echo ( "false" !== $share_buttons  ) ? do_shortcode( apply_filters("rt_social_share_shortcode","[rt_social_media_share]") ) : "";
								...
								//SKU
								echo ! empty( $sku ) ? sprintf( '<span class="sku_wrapper"><span class="sku"><b>%s:</b> %s  </span></span> ', esc_html_x('SKU','Frontend Text','rt_theme'), $sku ) : "" ;
								

								//categories 
								$get_terms = get_the_terms( $post->ID , 'product_categories');
								$category_count = is_array( $get_terms ) ? count( $get_terms ) : 0;
								echo '<span class="posted_in">' . get_the_term_list( $post->ID, 'product_categories', '<b>'.esc_html(_n('Category','Categories', $category_count,'rt_theme')) .':</b> ', ', ', '' ) . '</span>' ; 
								echo '</div>';
							...
							<span itemprop="offers" itemscope itemtype="http://schema.org/Offer"><meta itemprop="price" content="<?php echo esc_attr($sale_price);?>"></span>
					</div><!-- end .col -->	
			...
			<?php 
				...
				if( $tabbed_page ){
					...
					if( $content_style != "4" ){ // Tabular style
						...
						$tabular_content =  $content ? sprintf('[rt_tab id="tab-general-details" title="%2$s" icon_name="icon-doc-alt"]%1$s[/rt_tab]', $content, esc_html_x('General Details','Frontend Text','rt_theme') ) : "";
						...
						// Attached Documents 		 	 
						$tabular_content .=  $attached_documents ? sprintf('[rt_tab id="tab-attached-documents" title="%2$s" icon_name="icon-docs"]%1$s[/rt_tab]', get_attached_documents( $attached_documents ), esc_html_x('Documents','Frontend Text','rt_theme') ) : "";


						// Comments 		
						$tabular_content .=  comments_open() ? sprintf('[rt_tab id="tab-comments" title="%2$s" icon_name="icon-chat-empty"]%1$s[/rt_tab]', "[rt_get_commnets_template]", esc_html_x('Comments','Frontend Text','rt_theme')) : "";
						...
						if ( is_array( $related_products ) ){
							...
							$tabular_content .=  sprintf('[rt_tab id="tab-related-products" title="%2$s" icon_name="icon-link"]%1$s[/rt_tab]', $related_products_shortcode, esc_html_x('Related Products','Frontend Text','rt_theme')); 
						...
						// Main content - General Details Tab	
						$accordion_content =  $content ? sprintf('[rt_accordion_content id="accordion-general-details" title="%2$s" icon_name="icon-doc-alt"]%1$s[/rt_accordion_content]',  apply_filters('the_content', $content), esc_html_x('General Details','Frontend Text','rt_theme') ) : "";
						...
						// Attached Documents 		 	 
						$accordion_content .=  $attached_documents ? sprintf('[rt_accordion_content title="'.esc_html_x('Documents','Frontend Text','rt_theme').'" icon_name="icon-docs"]%s[/rt_accordion_content]', get_attached_documents($attached_documents)) : "";


						// Comments 		
						$accordion_content .=  comments_open() ? sprintf('[rt_accordion_content title="'.esc_html_x('Comments','Frontend Text','rt_theme').'" icon_name="icon-chat-empty"]%s[/rt_accordion_content]', "[rt_get_commnets_template]") : "";
						...
						if ( is_array( $related_products ) ){
							...
							$accordion_content .=  sprintf('[rt_accordion_content title="'.esc_html_x('Related Products','Frontend Text','rt_theme').'" icon_name="icon-link"]%s[/rt_accordion_content]', $related_products_shortcode);							
						}
